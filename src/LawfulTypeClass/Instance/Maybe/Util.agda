...
equalityMaybe' {{ _ }} {{ lEq }} (Just x) (Just y) refl
    = IsLawfulEq.equality' lEq x y refl

compMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
...
transMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
...
transMaybe {{ _ }} {{ lOrd }} (Just x) (Just y) (Just z)
    = cong (λ o → o /= GT) (IsLawfulOrd.compareLt lOrd x z {!   !})

reflMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
    → ∀ (x : Maybe a) → (x <= x) ≡ True
reflMaybe Nothing = refl
reflMaybe {{ _ }} {{ lOrd }} (Just x)
    = cong (λ o → o /= GT) (IsLawfulOrd.compareEq lOrd x x refl)

antisymMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
    → ∀ (x y : Maybe a) → {{ (x <= y) ≡ True }} → {{ (y <= x) ≡ True }} → x ≡ y
antisymMaybe Nothing Nothing = refl
antisymMaybe {{ _ }} {{ lOrd }} (Just x) (Just y) {{ h1 }} {{ h2 }}
    = -- ?
    cong Just {!   !}
    -- (IsLawfulOrd.antisymmetry lOrd x y ? ?)
    -- (IsLawfulEq.equality (IsLawfulOrd.super lOrd) x y {!   !})

lte2gteMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
    → ∀ (x y : Maybe a) → (x >= y) ≡ (y <= x)
...
lNotLteNeqMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
...
lt2gtMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
    → ∀ (x y : Maybe a) → (x > y) ≡ (y < x)
...
compareEqMaybe {{ _ }} {{ lOrd }} (Just x) (Just .x) refl
    = IsLawfulOrd.compareEq lOrd x x refl
...
min2ifMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
...
max2ifMaybe : {{ iOrdA : Ord a }} → {{ iLawfulOrdA : IsLawfulOrd a }}
